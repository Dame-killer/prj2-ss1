                ...
                <tbody>
                    @forelse ($class_subject_students as $css)
                        <tr>
                            ...
                            <td>{{ $css->class_name}}-{{ $css->subject_name }}</td>
                            <td>{{ $css->exam_times }}</td>
                            <td>
                                <div class="row">
                                    <div class="col-3">
                                        <form action="{{ route('aa-css-del') }}" method="POST">
                                            @csrf
                                            <input name="cs_id" hidden value="{{ $css->css_id }}">
                                            <button class="btn btn-danger">Xóa</button>
                                        </form>
                                    </div>
                                    <div class="col-5">
                                        <form action="{{ route('aa-css-edit') }}" method="GET">
                                            @csrf
                                            <input name="cs_id" hidden value="{{ $css->css_id }}">
